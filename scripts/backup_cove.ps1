<#
.SYNOPSIS
Verdel NinjaRMM script – Cove Data Protection backup status check

.DESCRIPTION
Checks Cove Data Protection backup health using ClientTool.exe and StatusReport.xml.
Collects service/process state, configured datasources, last session outcomes, last success age,
error details, and Local Speed Vault synchronization status. Outputs to console or writes to a
NinjaOne custom field for monitoring and alerting.
...
.CREDITS / ATTRIBUTION
This script is based on, and includes adapted logic from, the "Backup-Scripts" repository by BackupNerd:
https://github.com/BackupNerd/Backup-Scripts

Modifications include (non-exhaustive):
- Adjustments for NinjaOne output and custom field writing
- Parameterization and thresholds aligned to Verdel monitoring standards
- Output formatting changes and additional guardrails for RMM execution
...
Cove Data Protection (Backup Manager) installed
NinjaOne RMM (Custom Field integration)

.PARAMETERS
- Datasource: Specific datasource/plugin to evaluate (dynamic set based on Cove types)
- ServerSuccessHoursInVal: Max hours since last success for servers (default 24)
- WrkStnSuccessHoursInVal: Max hours since last success for workstations (default 72)
- SynchThreshold: Minimum acceptable sync percentage for Local Speed Vault (default 90)
- WriteToConsole: Write output to console instead of only capturing for NinjaOne

.OUTPUT
- Console output (optional via -WriteToConsole)
- NinjaOne custom field: covebackupresultlast28days
...
.LICENSE
MIT License
Third-party portions remain under the original repository license and terms (BackupNerd).
#>
...
function Check-CoveExists {
    $CoveExists = Test-Path "C:\Program Files\Backup Manager\ClientTool.exe" -PathType Leaf -ErrorAction SilentlyContinue
    return $CoveExists -ne $null
}
...
Function Convert-UnixTimeToDateTime($inputUnixTime){
    ...
    $epoch = Get-Date -Date "1970-01-01 01:00:00Z"
...
Write-BackupLog "12312312313.123123123" | RND KB       ## output = 12023742.49 KB
Write-BackupLog "TEST $(1231231231.2312 | RND GB 2)"   ## output = TEST 1.15 GB
#>
...
Function Get-BackupState {
    ...
    if ($BackupService.status -eq "Running"){
        ...
        $Script:BackupServiceOutTxt = $($BackupService.status)
        Write-BackupLog -MessageType Warning "Service Status   | $BackupServiceOutTxt"
        $global:failed = 1
        break
...
Function Get-Datasources {
    ...
    if ($Null -eq ($FunctionalProcess)) { 
        Write-BackupLog -Message "Backup Manager Not Running" -MessageType Warning
        $global:failed = 1 
    } else { 
        Do {
            ...
            if ($StatusValue -contains $BackupStatus) { 
                ...
                try {
                    ...
                    $script:Datasources = & $clienttool -machine-readable control.selection.list |
                        ...
                        Where-Object { $_.P2 -eq "Inclusive" } |
                        ForEach-Object { $_.P1 }
                    $Script:DataSourcesOutTxt = $Datasources -join ", "
                    Write-BackupLog -Message "====[Configured Datasources]===="
                    Write-BackupLog -Message "Data Sources              | $Script:DataSourcesOutTxt"
                } catch {
                    Write-BackupLog -Message "Error listing datasources: $_" -MessageType Warning
					Write-BackupLog -Message " "
                }
            }
...
Function Get-FixedVolumes {
    ...
    <#
    Write-BackupLog "Items that can be protected ??"
    ...
    Write-BackupLog "Items not able to be protected ??"
...
Function Get-Filters {
    ...
    if ($Null -eq ($FunctionalProcess)) { 
        Write-BackupLog -MessageType Warning "Backup Manager Not Running" 
    }
    else { 
                    ...
                    if ($StatusValue -contains $BackupStatus) { 
                        ...
                        try { # Command(s) to try
                            ...
                            If ($null -ne $Script:sessionerrors) {
                                ...
                                Write-BackupLog -MessageType Warning "[$datasource] Errors Found"
								
                                Write-BackupLog "Datum: $Script:ErrorDateTimeOutTxt Error: $Script:ErrorContentOutTxt Pad: $Script:ErrorPathOutTxt"
								$script:ErrorOutput += "[$datasource] Errors Found"
								$script:ErrorOutput += "Datum: $Script:ErrorDateTimeOutTxt Error: $Script:ErrorContentOutTxt"
								$global:failed = 1
                                ...
                                Write-BackupLog "`n[$datasource Errors] $Script:ErrorContentOutTxt`n"
                            } 
                        }
                        catch{ # What to do with terminating errors
                        }
...
Function Get-StatusReportBase {
    ...
    if (Test-Path $script:StatusReportxml ) {    
        ...
        Write-BackupLog "====[Device]===="
        Write-BackupLog "Machine Name              | $Script:MachineNameOutTxt"
        Write-BackupLog "Customer Name             | $Script:CustomerNameOutTxt"
        Write-BackupLog "OS Version                | $Script:OsVersionOutTxt"

        }  ## Test if StatusReport.xml file exists
    else{Write-BackupLog -MessageType Warning "[$script:StatusReportxml] not found"; $global:failed = 1}
    ...
    if (Test-Path $script:StatusReportxml ) {    
        ...
        if ($PluginColorBar) {
            ...
            if ($null -eq $PluginLastSuccessfulSessionStatus ) {$Script:LastSuccessStatusOutTxt = "Never"} else {
                ...
                Write-BackupLog "====[$Datasource Session]===="
                Write-BackupLog "28-Day Status             | $Script:PluginColorBarOutTxt"
                ...
                if ($LastSuccessTimeUTCOutTxt -gt 0) { 
                    ...
                    if ($HoursSinceLastOutVal -le $SuccessHoursInVal) {
                        Write-BackupLog "Last Success              | $HoursSinceLastOutVal(HRS) Ago"
                    }elseif ($HoursSinceLastOutVal -gt $SuccessHoursInVal) {
                        Write-BackupLog -MessageType Warning "Last Success     | $HoursSinceLastOutVal(HRS) Ago"
                        $global:failed = 1
                    }
                }
            }
            Write-BackupLog "Last Error Count          | $Script:LastErrorCountOutVal"

            if ($Script:LastErrorCountOutVal -ge 1) {Get-BackupErrors $DataSource 5}

        }else{Write-BackupLog -MessageType Warning "No Prior $Datasource Backup has completed";} ## Prevent null values
...
Function Get-Status ([int]$SynchThreshold) {
    ...
    Do {
        ...
        if ($StatusValue -contains $BackupStatus) { 
            ...
            try { 
                ...
                Write-BackupLog -MessageType Warning "ERROR     | $_"
                $global:failed = 1 
            }
        }
    }until (($Backupstatus -notcontains $StatusValue) -or ($retrycounter -ge 5)) 
    ...
    if (($LocalSpeedVaultEnabled -eq 1) -and (($BackupServerSynchronizationStatus -eq "Failed") -or ($LocalSpeedVaultSynchronizationStatus -eq "Failed"))) {Write-BackupLog -MessageType Warning "LSV Failed"; $global:failed = 1}
            
    elseif (($Script:LocalSpeedVaultEnabled -eq 1) -and ($BackupServerSynchronizationStatus -ne "synchronized")) { 
        if ( ($BackupServerSynchronizationStatus.replace("%","")/1 -lt $SynchThreshold )){Write-BackupLog -MessageType Warning "Cloud sync is below $SynchThreshold%"; $global:failed = 1}
    ...
    elseif (($LocalSpeedVaultEnabled -eq 1) -and ($LocalSpeedVaultSynchronizationStatus -ne "synchronized")) { 
        if ( ($LocalSpeedVaultSynchronizationStatus.replace("%",""))/1 -lt $SynchThreshold ){Write-BackupLog -MessageType Warning "LSV sync is below $SynchThreshold%"; $global:failed = 1}
    }  ## Warn if Sync % is below threshold
...
Function Format-OutputForConsole {
    param(
        [string]$content,
        [switch]$isHeader
    )
...
Function Write-BackupLog {
    ...
    Param(
        ...
        [string]$MessageType = 'Output'
    )

    # Check for section headers and add a newline before them if necessary
    if ($Message -match '^\[\w+.*\]$') {
        $Message = "====" + $Message + "====" # Add a newline before section headers
    }
...
if (Check-CoveExists) {
    ...
    Write-Host "Cove not found on the server. # exiting without error."
    Ninja-Property-Set covebackupresultlast28days "Cove not installed on the server."
    exit 0
}
...
foreach ($datasource in $datasources) {
    Switch ($script:datasource) { ## "FileSystem","SystemState","VMware","VssHyperV","VssMsSql","Exchange","MySql","NetworkShares","VssSharePoint"
        ...
        'MySql' {
            ...
            #Write-BackupLog "`n[Detecting $datasource DataSources]"
            if ($DatasourcePresent -eq $null) {Write-BackupLog -MessageType Warning "$Datasource configured but no $Datasource installation detected"; $global:failed = 1}
...
# At the very end, if $WriteToConsole is not specified, send the captured output to NinjaRMM
if (-not $WriteToConsole) {
	if ($global:failed -eq 1) {
		Write-BackupLog "Error during backup!"
		Write-BackupLog "$ErrorOutput"
		
	} else {
		Write-BackupLog "Backup was completed succesfully"
	}
    $combinedOutput = $backupOutput -join "`n"
    Ninja-Property-Set covebackupresultlast28days $combinedOutput
}
